  ...
  let(:runner) do
    ChefSpec::SoloRunner.new(
      ...
      %i(resource name action).each do |p|
        next if send(p).nil?
        node.default['openvpn_okta_resource_test'][p] = send(p)
      end
      properties.each do |k, v|
        next if v.nil?
        node.default['openvpn_okta_resource_test']['properties'][k] = v
      end
    end
  end
  let(:converge) { runner.converge('openvpn_okta_resource_test') }
